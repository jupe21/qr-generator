...
// Lokalni web UI za generiranje QR kod:
// lokalno: node server.js -> http://localhost:3777
...
async function handleGenerate(req, res) {
  ...
  try {
    ...
    return sendJson(res, 400, {
      error: `Neveljavna zahteva: ${err.message}`,
    });
  }
  ...
  try {
    ...
    if (body.centerType === "image" && body.imageData) {
      // data URL (base64) iz brskalnika -> Buffer
      const match = /^data:[^;]+;base64,(.+)$/.exec(body.imageData);

      if (!match) {
        throw new Error("Neveljaven format naložene slike");
      }
    ...
    // Lokalno naj še vedno shrani v mapo izhod.
    // Na Vercelu pa ni smiselno trajno shranjevat datotek, zato samo vrnemo base64 sliko.
